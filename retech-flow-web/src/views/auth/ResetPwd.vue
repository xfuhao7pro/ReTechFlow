<template>
  <AuthLayout>
    <div class="form-wrapper">
      ...
      <p class="welcome-sub">请输入您的邮箱和新密码</p>
...
const handleSendCode = async () => {
  ...
  try {
    const res = await authAPI.sendCodeAPI(resetForm.email)
    if (res.code === 200) {
...
const handleReset = async () => {
  ...
  await resetFormRef.value.validate(async (valid) => {
    if (valid) {
      loading.value = true;
      try {
        const res = await authAPI.resetPasswordAPI({
          email: resetForm.email,
          auth_code: resetForm.auth_code,
